...
//
// Chama up()/down() da migration DIRETAMENTE, em vez de
// conexao.undoLastMigration()/runMigrations(). A primeira versão deste
// arquivo usava undoLastMigration() — funcionava até a migration
// CreateAddresses entrar depois desta no histórico: "desfazer a última"
// passou a desfazer CreateAddresses, não MoneyToCents, e o teste quebrou por
// um motivo que não tinha nada a ver com dinheiro. Chamando a classe da
// migration direto, o teste fica correto não importa quantas migrations
// entrem depois dela.
//
...
describe('MoneyToCents (integração)', () => {
  ...
  const migration = new MoneyToCents1787900000006();
  ...
  beforeEach(async () => {
    ...
    runner = conexao.createQueryRunner();
