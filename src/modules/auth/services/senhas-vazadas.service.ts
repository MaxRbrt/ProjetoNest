...
@Injectable()
export class SenhasVazadasService {
  ...
  // completo nunca saem da aplicação. Falha fechada — se a verificação externa
  // não está confiável, o cadastro não prossegue com uma senha possivelmente
  // comprometida.
  // ---------------------------------------------
  async estaComprometida(password: string): Promise<boolean> {
    ...
    try {
      const response = await fetch(`${this.apiUrl}/range/${prefix}`, {
        headers: {
          'Add-Padding': 'true',
          'Usuario-Agent': 'projeto-test-auth/1.0',
        },
