...
import { opcoesDaFonteDeDados } from '../db/fonte-de-dados';
import { Papel, Usuario } from '../modules/usuarios/usuario.entity';
...
function confirmationFlag(target: DatabaseTarget): string {
  return `--confirm-target=${target.usuario}@${target.host}:${target.port}/${target.database}`;
}
...
// ---------------------------------------------
// Entrada da CLI
// Importar `data-source` carrega dotenv e lê process.env no próprio import.
// Isso antes era adiado com require() dentro da função, para que a suíte de
// testes pudesse importar os utilitários deste arquivo sem disparar o efeito;
// sem os testes, o adiamento perdeu a razão de ser e o import volta ao topo.
// ---------------------------------------------
async function main(): Promise<void> {
  const url = (opcoesDaFonteDeDados as { url?: string }).url;
  if (!url) {
    throw new Error(
  ...
  const message = await promoteAdmin(
    process.argv[2],
    () => new DataSource({ ...opcoesDaFonteDeDados, logging: false }),
    target,
    process.argv[3],
  );
  console.log(message);
